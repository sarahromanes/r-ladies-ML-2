...
rer.rpart <- length(which(fitted.vals!=y))/length(y)
rer.rpart

# Confusion matrix
confusion.rpart <- confusionMatrix(y, fitted.vals) 
...
rer.rf <- length(which(fitted.vals!=y))/length(y)
rer.rf

# Confusion matrix
confusion.rf <- confusionMatrix(y, fitted.vals) 
confusion.rf$table
...
# Resubstitution error rate
rer.knn <- length(which(fitted.vals!=y))/length(y)
rer.knn

# Confusion matrix
confusion.knn <- confusionMatrix(y, fitted.vals) 
confusion.knn$table
...
rer.glm <- length(which(fitted.vals!=y))/length(y)
rer.glm

# Confusion matrix
confusion.glm <- confusionMatrix(y, fitted.vals) 
confusion.glm$table
